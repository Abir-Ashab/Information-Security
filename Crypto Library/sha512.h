...
void message_schedeuler(string single_block)
{
    ...
    for (int i = 0; i < (int)single_block.length(); i += 64, ++nChunk)
        Message[nChunk] = getDecFromBin(single_block.substr(i, 64));
...
string SHA(string msg)
{
    ...
    cout << "The binary string of main string : " << binary_str << endl;
    ...
    for (int i = 0; i < pad_len - 129; i++)
        binary_str += "0";
    string lengthbits =
        bitset<128>(binary_msglen).to_string();
    ...
    for (int i = 0; i < binary_str.length(); i += 1024, ++iBlock)
        Blocks[iBlock] = binary_str.substr(i, 1024);
...
void sha512()
{
    ...
    cout << "\nThe main string is : "<< main_string << endl;
    string final_hashvalue = SHA(main_string);
    cout << "\nThe hash value is : " << final_hashvalue << endl;
}